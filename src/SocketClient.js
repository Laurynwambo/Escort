...
const SocketClient = () => {
  ...
  //!Comments
  useEffect(() => {
    socket.on("createUserToClient", (newUser) => {
      dispatch({ type: POST_TYPES.UPDATE_POST, payload: newUser });
    });
    return () => socket.off("createUserToClient");
  }, [socket, dispatch]);

  useEffect(() => {
    socket.on("deleteUserToClient", (newUser) => {
      dispatch({ type: POST_TYPES.UPDATE_POST, payload: newUser });
    });
    return () => socket.off("deleteUserToClient");
  }, [socket, dispatch]);
